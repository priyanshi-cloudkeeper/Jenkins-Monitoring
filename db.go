...
func initDB() {
	// Read DATABASE_URL here, after env_loader.go's init() has run
	connStr := os.Getenv("DATABASE_URL")
	if connStr == "" {
		connStr = "postgres://jenkins:jenkins@localhost:5432/jenkins-data?sslmode=disable"
...
func createTables() {
	// ... (rest of createTables remains the same)
	jobsTableSQL := `
